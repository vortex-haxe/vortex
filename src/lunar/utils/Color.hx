...
class Color {
    ...
    public static overload extern inline function create(color:String) {
        final colorStr:String = (cast color).replace("#", "").replace("0xFF", "").replace("0x", "");
    ...
    public static overload extern inline function create(hsv:HSVColor) {
        final newColor:Color = {};

        var i:Int;
        var f:Float, p:Float, q:Float, t:Float;
        newColor.a = hsv.a ?? 1.0;
    
        if(hsv.s == 0.0) {
            // Achromatic (gray)
            newColor.r = newColor.g = newColor.b = hsv.v;
            return newColor;
        }
